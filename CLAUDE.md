...
5. **Minimize `reinterpret_cast` — it's a debt metric.** Every `reinterpret_cast` means a type hasn't been recovered yet. If your diff increases the cast count, you're adding tech debt, not reducing it. Define the struct in `include/` so you can use typed access instead. The reviewer tracks cast density — diffs above 10% get flagged.
